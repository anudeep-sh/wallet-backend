...
router.put("/config", walletAuth, adminOnly, async (ctx: any) => {
  ctx.body = await commissionService.setConfig(
...
router.get("/earnings", walletAuth, async (ctx: any) => {
  ...
  ctx.body = await commissionService.getEarnings(
    ctx.state.walletUser.userId,
    page,
    limit,
  );
...
/** GET /all-overrides — admin: list ALL user overrides with names */
router.get("/all-overrides", walletAuth, adminOnly, async (ctx: any) => {
  ctx.body = await commissionService.getAllOverrides();
  ctx.status = 200;
});

/** GET /user-chain/:userId — admin: commission chain for a specific user */
router.get("/user-chain/:userId", walletAuth, adminOnly, async (ctx: any) => {
  ctx.body = await commissionService.getUserCommissionChain(ctx.params.userId);
  ctx.status = 200;
});
